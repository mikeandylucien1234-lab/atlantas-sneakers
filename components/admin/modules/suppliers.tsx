        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/50" onClick={() => setWizard(null)}>
          <div className={cn("w-full max-w-2xl rounded-[18px] border p-5 space-y-4 max-h-[92vh] overflow-y-auto", p, brd)} onClick={e => e.stopPropagation()}>
            {wizard.loading ? <div className="p-10 flex justify-center"><Loader2 className={cn("w-6 h-6 animate-spin", sub)} /></div> : (<>
              ...
              {wizStep === 3 && (() => { const cost = Number(wizard.supplier_price) || 0; const suggested = +(cost * 1.35).toFixed(2); return <div className="space-y-3"><div className={cn("rounded-[10px] border p-3", brd)}><p className={cn("text-xs", sub)}>Supplier cost: <b className={txt}>${cost.toFixed(2)}</b> · default rule suggests <b className={txt}>${suggested}</b></p></div><div className="grid grid-cols-2 gap-3"><div><label className={labelCls}>Sale Price ($)</label><input type="number" value={wizard.price ?? suggested} onChange={e => setWizard(w => ({ ...w, price: parseFloat(e.target.value) || 0 }))} className={inpCls} /></div><div><label className={labelCls}>Compare-at ($)</label><input type="number" value={wizard.compare_price ?? +(suggested * 1.3).toFixed(2)} onChange={e => setWizard(w => ({ ...w, compare_price: parseFloat(e.target.value) || 0 }))} className={inpCls} /></div></div><p className={cn("text-[11px]", sub)}>Profit: <b className="text-emerald-600">${(((wizard.price ?? suggested) - cost)).toFixed(2)}</b> ({cost ? Math.round((((wizard.price ?? suggested) - cost) / cost) * 100) : 0}% margin)</p></div>; })()}
